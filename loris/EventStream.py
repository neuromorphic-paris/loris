...
class EventStream(object):
    ...
    def parse_file(event_data):
        ...
        if stream_type is 0:
            ...
            return DVSStream.read(event_data, stream_version, width, height, file_cursor, end, events, current_time, bar)
        elif stream_type is 2:
            return ATISStream.read(event_data, stream_version, width, height, file_cursor, end, events, current_time, bar)
    ...
    def write_dvs_file(self, filename):
        """
        """
    ...
    def write_atis_file(self, filename):
        if not os.path.exists(os.path.dirname(filename)):
            try:
                os.makedirs(os.path.dirname(filename))
            except OSError as exc:  # Guard against race condition
    ...
    def write_generic_file(self, filename):
        """
    ...
    def write_amd_file(self, filename):
        """
        """
        if not os.path.exists(os.path.dirname(filename)):
            try:
    ...
    def write_color_file(self, filename):
        """
        """
        if not os.path.exists(os.path.dirname(filename)):
            try:
